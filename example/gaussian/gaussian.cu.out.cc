 ...
 **-------------------------------------------------------
 */
ocl_kernel_prop Fan1()
{
    ocl_kernel_prop prop;
    prop.src = R"NKT(#include "functions.h"
    __kernel void Fan1(__global float *m_cuda, __global float *a_cuda, int Size, int t)
    {
        if(get_local_id(0) + get_group_id(0) * get_local_size(0) >= Size-1-t) return;
        *(m_cuda+Size*(get_local_size(0)*get_group_id(0)+get_local_id(0)+t+1)+t) = *(a_cuda+Size*(get_local_size(0)*get_group_id(0)+get_local_id(0)+t+1)+t) / *(a_cuda+Size*t+t);
    })NKT";
    prop.kernel_name = "Fan1";
    prop.args_num = 4;
...
ocl_kernel_prop Fan2()
{
    ocl_kernel_prop prop;
    prop.src = R"NKT(#include "functions.h"
    __kernel void Fan2(__global float *m_cuda, __global float *a_cuda, __global float *b_cuda,int Size, int j1, int t)
    {
        if(get_local_id(0) + get_group_id(0) * get_local_size(0) >= Size-1-t) return;
        if(get_local_id(1) + get_group_id(1) * get_local_size(1) >= Size-t) return;
...
void ForwardSub()
{
    ...
    gettimeofday(&time_start, NULL);
	for (t=0; t<(Size-1); t++) {
        ocl_kernel_prop prop_1 = Fan1();
        int flag = Size-t;
        std::pair<void*, unsigned long long> fuc1((void*)m_cuda, Size * Size * sizeof(float));
        std::pair<void*, unsigned long long> fuc2((void*)a_cuda, Size * Size * sizeof(float));
        std::pair<void*, unsigned long long> fuc3((void*)b_cuda, Size * sizeof(float));
        void *param_1[] = {(void*)&fuc1, (void*)&fuc2, (void*)&Size, (void*)&t};
        ...
        ocl_kernel_prop prop_2 = Fan2();
        void *param_2[] = {(void*)&fuc1, (void*)&fuc2, (void*)&fuc3, (void*)&Size,(void*)&flag, (void*)&t};
        dim3 grid_dim_2(dimGridXY);
        dim3 block_dim_2(dimBlockXY);
        cuLaunchKernel((void*)&prop_2, grid_dim_2.x, grid_dim_2.y, grid_dim_2.z, block_dim_2.x, block_dim_2.y, block_dim_2.z, 0, nullptr, param_2, nullptr);
    }
	// end timing kernels
	struct timeval time_end;
    gettimeofday(&time_end, NULL);
