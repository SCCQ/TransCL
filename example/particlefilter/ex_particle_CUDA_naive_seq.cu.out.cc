...
* param7: Nparticles
*****************************/
 ocl_kernel_prop kernel1(){
ocl_kernel_prop prop;
prop.src = R"NKT(#include "functions.h"
__kernel void kernel1(__global double * arrayX, __global double * arrayY, __global double * CDF, __global double * u, __global double * xj, __global double * yj, int Nparticles){
 int block_id = get_group_id(0);// + gridDim.x * blockIdx.y;
 int i = get_local_size(0) * block_id + get_local_id(0);
...
void particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles){
	...
	CUdeviceptr * arrayX_GPU;
	CUdeviceptr * arrayY_GPU;
	CUdeviceptr * xj_GPU;
	CUdeviceptr * yj_GPU;
	CUdeviceptr * CDF_GPU;
	...
	for(k = 1; k < Nfr; k++){
		...
		ocl_kernel_prop prop = kernel1();
		std::pair<void*, unsigned long long> fuc1((void*)arrayX_GPU, sizeof(double)*Nparticles);
		std::pair<void*, unsigned long long> fuc2((void*)arrayY_GPU, sizeof(double)*Nparticles);
		std::pair<void*, unsigned long long> fuc3((void*)CDF_GPU, sizeof(double)*Nparticles);
		std::pair<void*, unsigned long long> fuc4((void*)u_GPU, sizeof(double)*Nparticles);
		std::pair<void*, unsigned long long> fuc5((void*)xj_GPU, sizeof(double)*Nparticles);
		std::pair<void*, unsigned long long> fuc6((void*)yj_GPU, sizeof(double)*Nparticles);
        void *param[] = {(void*)&fuc1, (void*)&fuc2, (void*)&fuc3, (void*)&fuc4, (void*)&fuc5, (void*)&fuc6, (void*)&Nparticles};
        cuLaunchKernel((void*)&prop, num_blocks, 0, 0, threads_per_block, 0, 0, 0, 0, param, 0);
